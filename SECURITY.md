...
Starting with `0.1.3`, release tarballs include a SHA-256 checksum and GitHub build-provenance attestation. Verify the current archive with `sha256sum -c` and `gh attestation verify dsh-promptwall-0.1.6.tgz --repo Chhlafiu4312/promptwall` before installing it in a sensitive environment.
