...
- `src/scanner.ts`: multilingual prompt-injection rules, scoring, and quarantine;
- `src/secrets.ts`: high-confidence secret detection and redaction;
- `src/policy.ts`: DSH pre/post tool decisions;
- `src/tool.ts`: `promptwall_scan` schema and receipt rendering;
